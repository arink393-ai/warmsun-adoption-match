...
這次是一次全面的安全審查加上五個新功能，重新貼一次 `supabase-schema.sql` 才會生效。

**安全性補強**（都用真的 Postgres 16 起一個乾淨的資料庫，模擬攻擊者直接呼叫 RPC／繞過畫面
的方式驗證過，不只是看程式碼）：

- `apply_to()`：原本只有畫面上「已經申請過」的按鈕會擋，直接呼叫這支 RPC 沒有任何防護，
  可以對同一個人重複送出無限筆申請——補上伺服器端的重複申請檢查，同時申請對象也一併排除
  已被停權（`banned`）的帳號。
- `skip_to_unlock()`：扣點前少了 `for update` 列鎖，跟其他所有扣點函式的寫法不一致，理論上
  併發呼叫可能造成點數扣兩次卻只算一次——補上鎖。
- `reports`（檢舉）：原本沒有任何防灌爆機制，同一個人可以無限次檢舉同一個對象——加上
  partial unique index，同一組檢舉人／被檢舉人只要還有一筆「尚未處理」的檢舉就不能再送出
  第二筆，畫面上會顯示「你已經檢舉過這個人了，正在等待管理員處理」。
- 清掉一段已經死掉、殘留在檔案裡容易誤導人的舊版 `apply_to()` 覆載（參數不同的舊函式）。

**對話視窗（免費，第二階段就開放）**：雙方進入第二階段之後，出訊息列表旁會多一個「開始
對話」按鈕，點開是一個簡單的訊息串（不是全站私訊，綁定在這筆申請底下），可以問對方客製化
的感情問題。跟原本付費才能看到的「日常觀察」文字解鎖是兩個並存的功能，不互相取代。新增
`messages` 資料表，RLS 只允許申請的雙方（`stage >= 2`）互看／各自發言，管理員可以看但不能
代發，被停權的帳號不能再發新訊息。
...
**管理後台：會員管理**：管理後台新增「會員管理」清單，可以搜尋所有會員、把違規的帳號標記
為「停權」（會要求輸入原因，停權後對方看板上會顯示自己的帳號已被停權，也看不到自己出現在
公開佈告欄）、解除停權，或直接移除對方的登記資料。停權狀態受資料庫層的保護觸發器保護，只有
`admin_set_banned()` 這支限定管理員呼叫的函式能改。

**AI 生成照片規則**：上傳大頭照時多一個「這是 AI 生成的照片」勾選框，使用者自行誠實勾選。
勾了之後審核說明會顯示「AI 生成照片需要人工比對相似度是否達 95% 以上，審核大約需要 7 個
工作天」；沒勾（本人真實照片）維持原本較快的審核速度。管理後台的照片審核佇列在遇到勾選
AI 生成的項目時，會多顯示一個提醒 banner，提示管理員要手動比對相似度。
...
- 檢舉目前只會列進管理後台讓你人工判斷；管理後台現在可以手動停權／移除對方的登記
  （見第 16 節），但沒有自動封鎖機制，還是需要人工判斷後手動操作。
